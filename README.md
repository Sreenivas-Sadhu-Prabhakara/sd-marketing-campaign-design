...
| **Asset Type** | Marketing Campaign |
| **Control Record** | Marketing Campaign Design |
...
| POST | `/v1/marketing-campaign-design/initiate` | Initiate |
| GET | `/v1/marketing-campaign-design` | Retrieve (list) |
| GET | `/v1/marketing-campaign-design/{crId}/retrieve` | Retrieve |
| PUT | `/v1/marketing-campaign-design/{crId}/update` | Update |
| PUT | `/v1/marketing-campaign-design/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/marketing-campaign-design/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-marketing-campaign-design:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-marketing-campaign-design ./helm -n bian-sales-service
```

Exposed through the platform Gateway at path prefix `/sd-marketing-campaign-design` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
